...
class Background:
    def __init__(self):
        ...
        gpuBackground= es.toGPUShape(bs.createTextureQuad("images/background.jpg"), GL_REPEAT, GL_NEAREST)
...
#Clase que se encaga de cargar la textura de fondo cuando se gana el juego y almacenar la información
class GameOver1:
    def __init__(self):
        ## Se carga la textura del game over 
        gpuGameOver= es.toGPUShape(bs.createTextureQuad("images/gameover1.png"), GL_REPEAT, GL_NEAREST)
        #Se crea el nodo asociado game over
...
#Clase que se encaga de cargar la textura de fondo cuando se pierde el juego y almacenar la información

class GameOver2:
    def __init__(self):

        ## Se carga la textura del game over 
        gpuGameOver= es.toGPUShape(bs.createTextureQuad("images/gameover2.png"), GL_REPEAT, GL_NEAREST)
...
class Model:
    def __init__(self,n):
        ...
        spaceInvaders.childs += enemies.nodes
        #Atributos de la clase
        ...
        self.shot = shot 
        self.gameover1 = gameover1
        self.gameover2= gameover2
        self.enemyshot= enemyshot
        self.countEnemy = 0
        #Cantidad de enemigos que quedan por poner 
        self.n = n-1
        #Cantidad de enemigos 
        self.enemiesslives = n
        #Vidas del jugador
        self.shipslives = 3
    ...
    def addEnemy(self):
        self.countEnemy += 1
        if self.countEnemy == 20:
            self.countEnemy = 0
            a = randint(0,2)

            if a == 0:
                self.enemies.createEnemy(-0.9)
                self.n -= 1
    ...
    def addEnemyShot(self):
        self.enemyshot.createEnemyShot(self.enemies)

    #Función que, dado un índice, elimina al enemigo asociado al índice
    def eliminateEnemy(self,i):
        self.enemies.removeEnemy(i)
    ...
    def crashShotEnemy(self):
        for i in range(len(self.shot.y)):
            for j in range(len(self.enemies.y)):
                if self.shot.y[i] > self.enemies.y[j] -0.01 and self.shot.y[i] < self.enemies.y[j] + 0.01:
                    if self.shot.x[i] > self.enemies.x[j] - 0.05 and self.shot.x[i] < self.enemies.x[j] + 0.05:
                        #En caso de existir colisión, se elimina el disparo y la nave enemiga de la pantalla
                        self.eliminateEnemy(j)
                        self.eliminateShot(i)
                        self.enemiesslives -=1
                        return
    ...
    #Función que verifica si un enemigo colisiona con la nave del jugador
    def crashEnemyShip(self):
        for i in range(len(self.enemies.y)):
            if self.enemies.y[i] > self.ship.y -0.05 and self.enemies.y[i] < self.ship.y + 0.05:
                if self.enemies.x[i] > self.ship.x - 0.05 and self.enemies.x[i] < self.ship.x + 0.05:
                    #En caso de existir colisión, se elimina el disparo enemigo de la pantalla y el jugador pierde una vida 
                    self.shipslives = 0
                    return
    ...
    def update(self,dx,dy,shot):
        if self.enemiesslives == 0:
            self.scene.childs = [self.gameover1.node]
        elif self.shipslives== 0:
            self.scene.childs = [self.gameover2.node]
            ...
            #Si una nave enemiga llega a la parte inferior de la pantalla.
            if self.enemies.y[0] < -0.8:
                self.shipslives = 0
            ...
            self.scene.childs += self.enemies.nodes
